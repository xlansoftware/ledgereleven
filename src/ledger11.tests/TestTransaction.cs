...
public class TestTransaction
{
    ...
    [Fact]
    public async Task TransactionController_Update2()
    {
        // Arrange
        using var serviceProvider = await TestExtesions.MockLedgerServiceProviderAsync("xuser1");
        ...
        // Act 1: Create a new transaction with currency and exchange rate
        var newTransaction = new Transaction
        {
            Value = 50,
            Date = DateTime.UtcNow,
        ...
        Assert.Equal(initialCategory.Id, createdTransaction.CategoryId);
        Assert.Equal("note 1", createdTransaction.Notes);
        Assert.Equal("USD", createdTransaction.Currency);
        Assert.Equal(1.05m, createdTransaction.ExchangeRate);

        // Act 2: Update the transaction with a new category and new FX data
        createdTransaction.Category = null;
        createdTransaction.CategoryId = updatedCategory.Id;
        createdTransaction.Notes = "Note 2";
        createdTransaction.Currency = "EUR";
        createdTransaction.ExchangeRate = 1.12m;
        ...
        Assert.Equal("Note 2", updatedTransaction.Notes);
        Assert.Equal(createdTransaction.Id, updatedTransaction.Id);
        Assert.Equal("EUR", updatedTransaction.Currency);
        Assert.Equal(1.12m, updatedTransaction.ExchangeRate);
    }
